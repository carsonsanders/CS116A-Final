...
public:
	...
	glm::vec3 updateKeyFrame(int currentFrame); //returns change in position 
	void addKeyFrame(KeyFrame* ptr, int currentFrame);

	std::vector<int> meshIndex;

	bool inBounds(glm::vec3 vert) {
		glm::vec3 topLeftCorner = position + glm::vec3(-2, 2, 2); // top left corner of bounding box, offset of position
		glm::vec3 botRightCorner = position + glm::vec3(2, -2, -2); // bottom far right corner of bounding box

		if (vert.x >= topLeftCorner.x && vert.x <= botRightCorner.x && vert.y <= topLeftCorner.y && vert.y >= botRightCorner.y && vert.z >= botRightCorner.z && vert.z <= topLeftCorner.z) {
			return true;
		}
		else {
			return false;
	...
	void drawBoundingBox() {
		//forward points
		glm::vec3 topLeftCorner = position + glm::vec3(-2, 2, 2);
		glm::vec3 topRightCorner = position + glm::vec3(2, 2, 2);
		glm::vec3 botLeftCorner = position + glm::vec3(-2, -2, 2);
		glm::vec3 botRightCorner = position + glm::vec3(2, -2, 2);

		//back points
		glm::vec3 BbotRightCorner = position + glm::vec3(2, -2, -2);
		glm::vec3 BtopRightCorner = position + glm::vec3(2, 2, -2);
		glm::vec3 BbotLeftCorner = position + glm::vec3(-2, -2, -2);
		glm::vec3 BtopLeftCorner = position + glm::vec3(-2, 2, -2);

		ofDrawLine(topLeftCorner, topRightCorner);
		ofDrawLine(topLeftCorner, botLeftCorner);
		ofDrawLine(botLeftCorner, botRightCorner);
		ofDrawLine(topRightCorner, botRightCorner);
		ofDrawLine(topLeftCorner, BtopLeftCorner);
		ofDrawLine(BtopLeftCorner, BtopRightCorner);
		ofDrawLine(topRightCorner, BtopRightCorner);
		ofDrawLine(BtopRightCorner, BbotRightCorner);
		ofDrawLine(BbotRightCorner, botRightCorner);
		ofDrawLine(BbotRightCorner, BbotLeftCorner);
		ofDrawLine(BtopLeftCorner, BbotLeftCorner);
		ofDrawLine(botLeftCorner, BbotLeftCorner);
	}
