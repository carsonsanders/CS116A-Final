...
void ofApp::setup() {
	...
	rootJoint.name = "first";
	joint.name = "second";
	//rootJoint.addChild(&childJoint);

	model1.loadModel("bird.obj");
	
	helper1 = model1.getModelMeshHelper(0);
	cachedMesh1 = &helper1->cachedMesh;


	//gui
	
	gui.setup();
	gui.add(slider.setup("Frame", 1, 1, 200));
	gui.add(del.setup("Delete Keyframes"));
	del.addListener(this, &ofApp::deleteKeyframes);
	gui.add(addjoint.setup("Add Joint"));
	addjoint.addListener(this, &ofApp::addJoint);
	gui.add(deletejoint.setup("Delete Joint"));
	deletejoint.addListener(this, &ofApp::deleteJoint);
	gui.add(bind.setup("Bind"));
	bind.addListener(this, &ofApp::bindVertices);
	gui.add(addKey.setup("Set Keyframe"));
	addKey.addListener(this, &ofApp::addKeyFrame);
	gui.add(hideJoints.setup(hidejoints));
...
void ofApp::update() {
	...
	for (SceneObject* obj : scene) {
		glm::vec3 change = obj->updateKeyFrame(frame);
		
		for (int index : obj->meshIndex) {
			cachedMesh1->getVertices().at(index).x += change.x;
			cachedMesh1->getVertices().at(index).y += change.y;
			cachedMesh1->getVertices().at(index).z += change.z;
		}
		/*
		if (obj->name == "first") {
			for (int i = 0; i < cachedMesh1->getNumVertices(); i++)
			{
				cachedMesh1->getVertices().at(i).x += change.x;
				cachedMesh1->getVertices().at(i).y += change.y;
				cachedMesh1->getVertices().at(i).z += change.z;
			}
		}

		if (obj->name == "second") {
			for (int i = 0; i < cachedMesh2->getNumVertices(); i++)
			{
				cachedMesh2->getVertices().at(i).x += change.x;
				cachedMesh2->getVertices().at(i).y += change.y;
				cachedMesh2->getVertices().at(i).z += change.z;
			}
		}
		*/
	...
	/**
	if (keyFrameSet() && (frame >= key1.frame && frame <= key2.frame)) {
	...
	*/
	...
	//cachedMesh1->getVertices().at(25).x += 10;

	/* flapping effect
	if (frameCount <= 30) {
		cachedMesh1->getVertices().at(17).y -= .02;
		cachedMesh1->getVertices().at(85).y -= .02;
		cachedMesh1->getVertices().at(87).y -= .02;
		cachedMesh1->getVertices().at(124).y -= .02;

		cachedMesh2->getVertices().at(17).y -= .02;
		cachedMesh2->getVertices().at(85).y -= .02;
		cachedMesh2->getVertices().at(87).y -= .02;
		cachedMesh2->getVertices().at(124).y -= .02;
	}
	if (frameCount > 30) {
	...
	*/
	...
	helper1->vbo.updateMesh(*cachedMesh1);
...
void ofApp::draw() {
	...
	model1.setScale(.010, .010, .010);
	model1.setPosition(0, 0, 0);
	//model1.setRotation(0, 90, 0, 1, 0);
	model1.drawFaces();
